class User < ActiveRecord::Base
  ...
  def self.top_rated
    self.select('users.*')#select all attributes of the user
      ...
      .select('COUNT(DISTINCT comments.id) + COUNT(DISTINCT posts.id) AS rank') # Add the number of comments to the post and label the sum as "rank"
      ...
      .group('users.id') # Instructs the database group to group the results so that each user will be returned in a distinct row
